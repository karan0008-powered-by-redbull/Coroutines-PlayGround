...
class ContactsViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun fetchAllContacts() =
        ioThread { contactsLiveData.postValue(contactsDB.allContactsByNameList()) }

    fun insertContact(contact: Contact) = ioThread { contactsDB.insert(contact) }

    fun removeContact(contact: Contact) = ioThread { contactsDB.delete(contact) }

    fun updateContact(contact: Contact) = ioThread { contactsDB.update(contact) }

    fun getFavourites() =
        ioThread { favContactsLiveData.postValue(contactsDB.fetchFavouriteContacts()) }
